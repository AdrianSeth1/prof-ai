...
| Date | Summary |
|---|---|
| 2026-06-24 | Post-session artifact generation. New file `artifacts.py` with four generators: `generate_summary` (300-500 word prose recap), `generate_coverage_report` (wraps `gaps_stream`), `generate_study_guide` (concept-by-concept ##-headed markdown), `generate_quiz` (5 MC + 3 short-answer + 2 essay). Also `generate_artifacts(session_id, on_progress)` orchestrator and `list_artifacts(session_id)` which checks `exports/{session_id}/`. DOCX export via python-docx (already installed) — `_append_md_to_doc` handles ATX headings, bullets, numbered lists, bold/italic; graceful ImportError fallback. LLM model `qwen3:30b-a3b`, `think=False`, `keep_alive="30m"`; context caps `_TRANSCRIPT_MAX=12000`, `_CHUNKS_MAX=8000`. `api/main.py`: `stop` message now includes `session_id` in the `stopped` WS event. Three new REST endpoints: `POST /api/sessions/{id}/artifacts` (streams NDJSON progress — `{type:"progress",stage,message}` then `{type:"done",files:[{label,name,url}]}`), `GET /api/sessions/{id}/artifacts` (returns `{files:[...]}`), `GET /api/sessions/{id}/artifacts/{filename}` (FileResponse with path traversal guard). `LiveLecture.tsx`: added `ARTIFACT_STAGES` module-level constant (5 stages). `generateArtifacts` and `resetArtifacts` callbacks defined BEFORE `handleWsMessage` (order matters — callbacks referenced in deps). Artifact panel shown when `!sessionInfo && artifactState !== 'idle'`: stage list (spinner for active, checkmark for done), download `<a>` tags when done, error box when error, "Start New Session" button resets to idle. `stopped` WS handler calls `generateArtifacts(endedId)` after clearing session state. `GapsAnalysis.tsx`: added `ArtifactFile` type and `artifactGenState` state; `useEffect` fetches `GET /api/sessions/{id}/artifacts` on session change; `handleGenerateArtifacts` POSTs to generate and streams NDJSON; artifacts section rendered below gap results with download links or a "Generate artifacts" button if none exist. Exports directory: `exports/{session_id}/` with `summary.md`, `coverage_report.md`, `study_guide.md`, `quiz.md`, and optionally `{session_id}_artifacts.docx`. Build: 0 TypeScript errors, Vite build clean. |
...
| `artifacts.py` | Post-session artifact generation: summary, coverage report, study guide, quiz. Writes to `exports/{session_id}/`. Called by `api/main.py` artifact endpoints. |
